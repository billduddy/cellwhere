<?php
     ...
     * 		and then output a html script which is capable to display the network in browsers.
     *
     * Variables:
     ...
     * 		$org_nodes: the protein IDs of only uploaded by user
    ...
    function xml_to_json($new_xml_file,$suffix,$list_comp,$org_nodes,$Symbol_added){
	global $upload_xml; // "yes": the xml file is uploaded by user 
	...
	/*------------------------------ php functions --------------------------------------*/
	    function json_node($out,$root_node,$parent,$next,$org_nodes,$Symbol_added){
		global $upload_xml;
		for($i=0;$i<count($root_node);$i++){
		    $nodes=$root_node[$i];
		    $label=$nodes["label"];
		    //echo $label;
		    if($Symbol_added&&array_key_exists((string)$label,$Symbol_added)){
			//echo "->Symbol_added:";
			$label=$Symbol_added[(string)$label];
			//echo "->$label<br/>";
		    }
			...
			if($label!=""&&!strstr($id,'local')){
				...
				foreach($nodes->att as $atts){
				    if($atts["name"]=="CellWhereUniprotID"){
					//echo "CellWhereUniprotID";
					fwrite($out,', "href":"http://www.uniprot.org/uniprot/'.$atts["value"].'"');
					//echo $atts["value"];
				    }
				}
			    }else{
			...
			json_node($out,$child,$parent,$next,$org_nodes,$Symbol_added);
			$parent=null;
		    }
		}
	    }
	...
	if($xml){
		...
		$root_node = $xml->node;
		$parent=null;
		json_node($out,$root_node,$parent,1,$org_nodes,$Symbol_added);
		//end of node
		fwrite($out,"]");
	    }
	    ///edge 
		...
		for($i=0;$i<count($xml->edge);$i++){
		    $edges=$xml->edge[$i];
		    $edge_id=$edges["id"];
		    $graphics = $edges->graphics;
		    if($graphics["fill"]!=null){
			$fill= $graphics["fill"];
			$width=(string)((int)$graphics["width"]*2);
			fwrite($out,",\n".'{"selector":"edge#'.$edge_id.'","css":{');
			fwrite($out,'"line-color":"'.$fill.'","width":"'.$width.'"}}');
			$edge_color=1;
		    }
		}
	   if($edge_color!=1){ //if edge has no color
