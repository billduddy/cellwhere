...
//============================================================ALL results=====================================================================
     ...
     */
     //-------------------------
     //$start = microtime(true);
     if($Source_Loc_Term=="GOonly"){
	 $result8= pg_query("SELECT QueryID,Symbol FROM results") or die("result6 error");		// Selects all from results table ,OurLocalization 
     }elseif (($Source_Loc_Term == "UniprotOnly") || ($Source_Loc_Term == "UniprotAndGO")) {
	 $result8= pg_query("SELECT QueryID,UniprotID FROM results") or die("result6 error");	
     }
     while($row = pg_fetch_array($result8)) {
	 if($row[1]){
	     $QueryID_Symbol[$row[0]] = explode("_",$row[1])[0];
	 }else{
	     $QueryID_Symbol[$row[0]] =$row[0];
	 }
     }
     /*
     $end = microtime(true);
     $time= $end - $start;
     $time = round($time, 2);      // Round to 2 decimal places
     echo "<br />--------result8 took:$time seconds.</br>";
     */
?>